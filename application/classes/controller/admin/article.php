...
class Controller_Admin_Article extends Controller_Admin_Admin {

/**вывод списка постов**/
	public function action_index()
	{
		
		$newsOrm = ORM::factory('articleorm');
	//pagination
	...
	//pagination
		$list = $newsOrm->limit($pager['limit'])->offset($pager['offset'])->find_all();
    		...
    		$this->template->scripts = array('/js/admin/article/list.js');
    		$this->template->path ="Публикации";
			$this->template->content = View::factory('admin/article/list.tpl', array("list"=>$list, "pager"=>$pager));
	...
	public function action_edit(){
		...
		if($id){
			...
			if($item[0]){
				...
				$this->template->scripts = array('/js/admin/arlicle/edit.js');
			$this->template->content = View::factory('admin/article/edit.tpl', array("post"=>$post));
			}
	...
	public function action_add(){
		$arr = array();
				$news = Model::factory("article");
				$this->template->scripts = array('/js/admin/article/edit.js');
				if(!empty($_POST)){ // сохранение поста
				...
				$this->template->path ="Статьи/создание";
				$this->template->content = View::factory('admin/article/edit.tpl', $arr);
	}
	...
	public function saveImg($tmp_img){
		...
		if(is_file($f_path)){
			...
			copy($f_path, $_SERVER['DOCUMENT_ROOT']."/userfiles/article_images/".$name);
	...
	public function imgFolder(){
		...
		return $_SERVER['DOCUMENT_ROOT']."/userfiles/article/";
	}
	...
	public function action_uploadimg(){
 		...
 		$this->template = View::factory('admin/article/upload_img.tpl');
	}
	/**сохранение файла*/
	public function action_saveimg(){
		
		$this->template = View::factory('admin/article/save_img.tpl');
	}
