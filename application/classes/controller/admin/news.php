...
/**вывод списка постов**/
	...
	//pagination
		$list = $newsOrm->limit($pager['limit'])->offset($pager['offset'])->find_all();
    		...
    		$this->template->scripts = array('/js/admin/news/list.js');
    		$this->template->path ="Публикации";
			$this->template->content = View::factory('admin/news/list.tpl', array("list"=>$list, "pager"=>$pager));
	...
	public function action_edit(){
		...
		if($id){
			...
			if($item[0]){
				...
				$this->template->scripts = array('/js/admin/news/edit.js');
			$this->template->content = View::factory('admin/news/edit.tpl', array("post"=>$post));
			}
	...
	public function action_add(){
		$arr = array();
				...
				$this->template->content = View::factory('admin/news/edit.tpl', $arr);
	}
	...
	public function saveImg($tmp_img){
		...
		if(is_file($f_path)){
			...
			copy($f_path, $_SERVER['DOCUMENT_ROOT']."/userfiles/news_images/".$name);
	...
	public function imgFolder(){
		...
		return $_SERVER['DOCUMENT_ROOT']."/userfiles/news_images/";
	}
	...
	public function action_uploadimg(){
 		...
 		$this->template = View::factory('admin/news/upload_img.tpl');
	}
	/**сохранение файла*/
	public function action_saveimg(){
		
		$this->template = View::factory('admin/news/save_img.tpl');
	}
